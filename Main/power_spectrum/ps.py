...
#Importing code for calculating of K
#from .KineticEnergyFraction import KineticEnergyFraction
...
#Equation 6 Estimation of R_s
def R_s(beta, vw):
    return (8*pi)**(1/3)/beta*vw

# Equation 6, but then estimation of Beta (most likely not needed)
def Beta(R_s, vw):
    return (8*pi)**(1/3)/R_s*vw
...
#Equation 20
def F_gw(g_s):
    g_s = 106.75 #Fixed during project
    return 3.57e-5*(100/g_s)**(1/3)

#Equation 30
def C(s):
    s = 1
    return s**3*(7/(4+3*s**2))**(7/2) #Which value do I pick for s?

#Equation 31
def fp_0(zp, g_s):
    zp = 10
    g_s = 106.75
    return 26*(1/H_s*R_s)*(zp/10)*(T_s/100)*(g_s/100)**1/6 #MilliHz

#Equation 29!!
def Omega_GW(alpha, beta, vw, ):
    return h*F_gw(106.75)*KineticEnergyFraction(0.2, 0.9)**2*(H_s*R_s(50, 0.9))/cs*10e-2*C(f/fp_0)(10, 106.75) #Check if C is written correctely

print(Omega_GW(0.2, 50, 0.9))
