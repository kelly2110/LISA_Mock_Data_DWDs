...
for alpha_value in sorted_unique_alpha_values:
    ...
    sorted_aic_ratio_values.extend(aic_ratio_values)
...
sorted_aic_ratio_values = np.array(sorted_aic_ratio_values)

# Filter data for non-NaN triangles
filtered_indices = ~np.isnan(sorted_aic_ratio_values)
filtered_alpha_values = sorted_alpha_values[filtered_indices]
filtered_vw_values = sorted_vw_values[filtered_indices]
filtered_aic_ratio_values = sorted_aic_ratio_values[filtered_indices]

# Create a contour plot for the ratio between AIC noise and AIC signal
plt.figure(figsize=(8, 6))
contour = plt.tricontourf(filtered_alpha_values, filtered_vw_values, filtered_aic_ratio_values, cmap='plasma')
colorbar = plt.colorbar(contour, label='AIC Noise / AIC Signal Ratio')
plt.xlabel(r'$\alpha$')
plt.ylabel(r'$v_w$')
plt.title(f'Contour Plot of AIC Noise / AIC Signal Ratio at Temperature {filter_temperature} GeV')
plt.show()
